...
%DIF ADD index.tex             Fri Mar 14 18:36:31 2025
...
\date{\DIFdelbegin \DIFdel{2024-11-01}\DIFdelend \DIFaddbegin \DIFadd{2025-03-14}\DIFaddend }
...
While there are heuristics, rules-of-thumb, and \DIFdelbegin \DIFdel{various }\DIFdelend guidelines (Allen \&
...
and visually attractive \DIFdelbegin \DIFdel{charts}\DIFdelend \DIFaddbegin \DIFadd{data displays}\DIFaddend , the best way to establish the
efficacy of various design decisions is to test \DIFdelbegin \DIFdel{charts: evaluate them
...
quickly becomes daunting or unaffordable}\DIFaddend . In this paper, we attempt to
distill the experience gained from conducting several different types of
graphics experiments (Hofmann et al., 2012; Robinson, 2022; VanderPlas
et al., 2019; Vanderplas et al., 2024; VanderPlas \& Hofmann, 2015,
2017), discussing the use of different testing methods (Vanderplas et
al., 2020), the process of designing a graphical experiment, and
analysis of the resulting empirical data. It is our hope that this paper
will lower the barriers that exist for conducting empirical graphics
research and reduce the probability of costly mistakes.
...
Shiny (Chang et al., 2021) as primary components\DIFdelbegin \DIFdel{, as well as JavaScript (including D3 (}\DIFdelend \DIFaddbegin \DIFadd{. In many experiments,
we customized the Shiny interface with JavaScript and d3 (}\DIFaddend Bostock et
...
}\DIFdelend (VanderPlas \& Hofmann, 2015):
...
data, embedded in }\DIFaddend an array of \DIFdelbegin \DIFdel{19 other }\DIFdelend innocent ``null'' plots \DIFdelbegin \DIFdel{that show
only noise}\DIFdelend \DIFaddbegin \DIFadd{generated through
resampling or simulation}\DIFaddend . If viewers consistently pick the target plot
at a higher rate than any of the \DIFdelbegin \DIFdel{19 }\DIFdelend null plots, the target plot is said to
be visually significant (Loy \& Hofmann, 2013; Majumder et al., 2013)
and a ``see'' value\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the visual analogue of a \(p\)-value \DIFdelbegin \DIFdel{for a statistical
test) can be calculated }\DIFdelend (Chowdhury et
al., 2020\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{), can be calculated using the }\texttt{\DIFadd{vinference}} \DIFadd{R package or
the process described in }\DIFaddend VanderPlas et al. \DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2021). \DIFaddbegin \DIFadd{The details of this
calculation are beyond the scope of this broader discussion of how to
test charts, but more detail on visual inference is provided in
\textless insert citation to visual inference WIRE article under
development\textgreater.
}
...
emphasize structural differences in the data \DIFdelbegin \DIFdel{and assign a hierarchy of visual attributes }\DIFdelend (VanderPlas \& Hofmann,
2017).
...
These \DIFdelbegin \DIFdel{indices then }\DIFdelend \DIFaddbegin \DIFadd{statistical indices may also }\DIFaddend serve as quantitative proxies \DIFdelbegin \DIFdel{of visual difficulty}\DIFdelend \DIFaddbegin \DIFadd{for the
difficulty of the visual task}\DIFaddend .

\DIFdelbegin \DIFdel{If using model based sampling methods, it is important to iteratively
...
and then simulate many data sets from each possible parameter combinations using a grid search. When sampling from a large data set, it is still important to assess the sample size and any potential
...
what numerical
statistic provides an estimation of the basic concept
which is being visually evaluated? }\DIFdelend \DIFaddbegin \DIFadd{To identify a statistical proxy for visual difficulty that may help with
narrowing the parameter space, it can be useful to consider numerical
measures used to estimate the same types of visual information that will
be assessed in the experiment. }\DIFaddend For instance, we have used
...
friends, and colleagues. These informal surveys provide extremely useful
feedback and can help to counteract the visual saturation of being
immersed in the design of a visualization experiment for months at a
time.
...
of our own experience: that of \DIFdelbegin \DIFdel{(}\DIFdelend VanderPlas \& Hofmann \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2017), where we
...
the effects shown in a single data set or phenomenon. The time and
effort invested in this step at the outset of the experiment pays
dividends when it allows for clear generalization of the experimental
results to an entire statistical concept rather than a single data set.
...
\DIFaddbegin \subsection{\DIFadd{Cognitive Load}}\label{cognitive-load}

\DIFaddend The primary human design factor to be aware of is that visual tasks and
...
\DIFaddbegin \subsection{\DIFadd{Accounting for Participant
Variability}}\label{accounting-for-participant-variability}

\DIFaddend While studies have found some relationship between lineup performance
...
\DIFaddbegin \subsection{\DIFadd{Trial Allocation to
Participants}}\label{trial-allocation-to-participants}
...
\DIFaddbegin \subsection{\DIFadd{Data Collection Platforms and
Infrastructure}}\label{data-collection-platforms-and-infrastructure}

\DIFaddend We have conducted visualization experiments using a wide variety of
tools: custom web \DIFdelbegin \DIFdel{servers running experiments using PHP black magic, Qualtrics surveys }\DIFdelend \DIFaddbegin \DIFadd{PHP servers running interactive forms for data
collection, generic web survey platforms (Qualtrics, Google Forms) }\DIFaddend for
static graphics, and \DIFdelbegin \DIFdel{writing full }\DIFdelend Shiny applications that control every part of the
experimental process from instructions to providing completion codes for
payment \DIFaddbegin \DIFadd{while rendering interactive graphics and generating fully
randomized data for each participant}\DIFaddend . In our research, Shiny has
provided the right balance between control over the experimental
setting, procedure, etc. and the intricate details of web server
management, however, this balance is likely different for every group
and potentially for every experiment. Likewise, we have recruited
participants using \DIFdelbegin \DIFdel{Amazon Mechanical Turk, Prolific, Reddit and other
}\DIFdelend \DIFaddbegin \DIFadd{services such as }\href{https://www.mturk.com/}{\DIFadd{Amazon
Mechanical Turk}} \DIFadd{and }\href{https://www.prolific.com/}{\DIFadd{Prolific}}\DIFadd{, on
}\DIFaddend social media sites
\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{(}\href{https://web.archive.org/web/20250228110244/https://www.reddit.com/r/SampleSize/}{\DIFadd{Reddit}}\DIFadd{,
Twitter(X), Mastodon, and BlueSky), and by }\DIFaddend email; each recruitment
method has trade-offs between cost, convenience, \DIFaddbegin \DIFadd{number of people in the
sampling frame, }\DIFaddend and control over \DIFdelbegin \DIFdel{demographic variables}\DIFdelend \DIFaddbegin \DIFadd{participant demographics}\DIFaddend . Rice et al.
(2024) recruited participants using \DIFdelbegin \DIFdel{NORC }\DIFdelend \DIFaddbegin \DIFadd{nationally representative }\DIFaddend panel
samples and found that \DIFdelbegin \DIFdel{demographics in a fully representative sample are extremely
important. }\DIFdelend \DIFaddbegin \DIFadd{conclusions from fully representative samples of
the population can be very different from volunteer samples recruited
using other services, as many people are unmotivated to engage with
charts, don't know how to read charts, or impose pre-formed conclusions
onto visual displays. These individuals are unlikely to participate in
most ad-hoc volunteer-based sampling methods. }\DIFaddend While using representative
\DIFdelbegin \DIFdel{panel-based sampling methods }\DIFdelend \DIFaddbegin \DIFadd{panel sampling products offered by organizations such as NORC or Gallup
}\DIFaddend is considerably more expensive \DIFdelbegin \DIFdel{, there are generalizability concerns with
online recruitment methods such as Prolific and Mechanical Turk}\DIFdelend \DIFaddbegin \DIFadd{than samples from Amazon MTurk or
Prolific, these results suggest that results from sampling methods used
online may not generalize to the broader population}\DIFaddend . People who
participate in studies via online platforms are more technologically
sophisticated and educated than the general population, and this bias
may significantly impact the conclusions.

\DIFaddbegin \DIFadd{If conducting an online experiment, we recommend consideration of
panel-based sampling methods as well as online testing platforms. While
Amazon MTurk was once the platform of choice for this type of research
(Heer \& Bostock, 2010), in 2025 Prolific, which is aimed at academic
experiments, seems to provide better options that ensure that
experiments are completed by people rather than AI than MTurk. It seems
likely that the preferred platform for participant recruitment will
continue to evolve over time, thus, we will recommend that experimenters
consider available options, compare pricing structures (as these vary
widely), and consider whether add-on fees for e.g.~demographically
representative samples are worth the additional cost. An additional
consideration when choosing between panel samples and online recruitment
platforms is the constraints placed on the types of questions and
graphics which can be used. In our experience, it is much easier to run
an experiment using the online recruitment platforms, which place many
fewer constraints on both the number and types of questions which can be
asked and the types of stimuli which can be used in the experiment.
}

\subsection{\DIFadd{Ethical Review of Human Subjects
Experiments}}\label{ethical-review-of-human-subjects-experiments}

\DIFadd{In almost any research environment, there are requirements that
experiments involving human participants undergo review by an ethics
board to ensure that participants are not harmed by participating in the
study. We are most familiar with regulations in the United States and
can only speak to that environment. Most graphics experiments conducted
in the US fall under the ``exempt'' category of experiments which
require only basic review and approval. These experiments record only
basic demographic information, ask participants to complete tasks that
...
demographic information is collected and stored separately from any
identifiable information, such as e.g.~user IDs from the participant
recruitment platform that allow us to monitor task completion and pay
participants for their time. Researchers should also ensure that they
comply with privacy laws such as the European
...
higher power than the Rasch models \DIFaddbegin \DIFadd{(Andrich, 1988; Lu et al., 2022)
}\DIFaddend which were historically recommended for these experiments. In addition,
similar model structures \DIFaddbegin \DIFadd{with different link functions }\DIFaddend can be used to
...
e.g.~}\DIFaddend anchor points.\DIFaddbegin \\
\DIFaddend The advantage of this approach is that it can accommodate extremely
messy data without requiring the extensive data cleaning\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, modeling of
heuristics like rounding and anchoring, and }\DIFaddend elimination of nonsensical
responses that might be necessary to fit a statistical model. \DIFaddbegin \DIFadd{While
...
\DIFaddbegin \bibitem[\citeproctext]{ref-schuttPainfreeAccurateBayesian2016}
