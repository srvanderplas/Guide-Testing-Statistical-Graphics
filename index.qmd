...
At the same time, more standard design considerations, like blocking, randomization, sample size, and analysis methodology are also important. 
...
Visualization experiments tend to opt for one of the following sampling strategies, ordered by generalizability to the wider population:
...
- In-person studies often recruit from undergraduate psychology or statistics students because researchers often have access to these populations and participation in studies may be part of a course's experiential learning activities, as in @vanderplasEscapingFlatlandGraphics2024. While the data generated from required classroom experiments may have higher variability, and it may be hard to generalize findings beyond undergraduate students, these studies can be conducted much more cheaply than studies conducted through online platforms or panels. 

- Social media and email-based recruitment. Researchers may post directly to social media sites such as Twitter/X, Mastodon, and BlueSky, forums and discussion sites such as Reddit, or general email lists that may be purchased from universities and other marketing organizations. If the goal is to recruit a specific population, such as meterologists or forensic examiners, forums may be an extremely effective way to recruit participants; we have used the [SampleSize subreddit](https://web.archive.org/web/20250228110244/https://www.reddit.com/r/SampleSize/) successfully and obtained participants that, while younger and more highly educated than those recruited from a platform, had similar results on visual estimation tasks [@vanderplasFramedReproducingRevisiting2019]. 
...
However, it is also reasonable to argue that this group of people may be less likely to use graphics at all in daily life. 
...
While Amazon MTurk was once the only large platform for this type of research [@heerCrowdsourcingGraphicalPerception2010], many researchers now prefer Prolific because it has stronger limitations on use of automation tools to complete tasks. 
We recommend that experimenters consider available options, compare pricing structures (as these vary widely), and consider whether add-on fees for e.g. demographically representative samples are worth the additional cost. 
It is also important to ensure that the platform supports the type of user engagement required for the study. 
In our experience, online recruitment platforms are more flexible and allow experimenters to use a much wider range of stimuli and experimental designs, but this sacrifices some ability to generalize results to a wider population. 
