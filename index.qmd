...
# Testing Methods and \ear{User} Engagement 
...
![This question asks users to write out a description of how the population of Ewoks changes over time, without any further cues, to determine whether participants default to multiplicative or additive \ear{language} descriptions.](Estimation_describe_plot_crop.png){#fig-estimation-describe}
...
![This question asks participants for a numerical estimate, but provides a basic calculator and scratchpad. All user interactions with the calculator and scratchpad are logged, providing insight into the user's thought process \ear{and estimation strategy}.](Estimation_numerical_screenshot_crop.png){#fig-estimation-calc}

In our experience, one of the most difficult components of designing an experiment which asks users to directly estimate information from a chart using a full scenario (background information, etc. as well as contextual details from the chart) is that the \ear{selection and} wording of such questions must be extremely carefully determined.
\ear{Mathematics education researchers have provided guidelines for selecting different levels of questioning in order to assess graph comprehension: literal reading of the data, reading between the data, and reading beyond the data \cite{wood1968objectives, curcio1987comprehension, friel2001making, glazer2011challenges}. In a recent study, we identified questions based on this framework for evaluating direct estimates and extending those estimates to make comparisons between two points.}
<!-- , and e -->
\ear{E}ven when great care is taken with the construction of the question, participant answer accuracy is fundamentally limited by the fact that many participants do not read and interpret the question with the care and precision that it was written.
Questions that ask participants to e.g. estimate the multiplicative \ear{change} in a quantity at two time points may be misunderstood as asking for an estimate of the additive difference, and the resulting estimates are then one or more orders of magnitude off of the correct answer.
...
Using JavaScript code modified from New York Times interactive graphics "You Draw It" features\cite{katzYouDrawIt2017}, we had users draw trend lines \ear{with their computer mouse} and make forecasts directly on interactive charts, with the data and user-drawn predictions recorded to our database.
