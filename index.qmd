...
# Experiment Development {#sec-exp-dev}
...
In addition to our brief overview of considerations in our research, @kochariConductingWebBasedExperiments2019 has many helpful suggestions for conducting web-based cognitive and perceptual experiments that also apply to studies of statistical graphics. 
...
- Attention checks - questions inserted into the experiment which appear similarly to actual trials but which instruct participants to select a specific response or complete a trivial task (e.g. select the result of 2+2). More guidance on attention checks can be found in @muszynskiAttentionChecksHow2023. 

- User input validation - mechanisms which reduce the potential parameter space of user inputs to a valid or reasonable set of parameters. Examples include not allowing participants to select negative variance values, requiring continuous user-drawn lines on an interactive chart, and even checking to ensure that the user's input is of the correct type. 
...
In this section, we broadly consider three different components of the experimental design process: infrastructure for data collection and participant recruitment, experimental design, and ethics panel review. 
...
Data collection process and infrastructure considerations can be complex in graphics experiments, but they are critically important for ensuring that data is successfully collected and that results can be generalized to the wider population. 
However, there are also several considerations that are related to more standard experimental design concerns, but with twists unique to cognitive science experiments. 
Experimenters must consider factors like cognitive load when determining how to block and randomize factors and allocate trials to participants. 
These factors are also extremely important when deciding on the number of participants necessary for an experiment.
...
Tasks which are more interactive, such as `You Draw It', may be somewhat easier for participants, but it is unlikely that participants would be willing to complete more than about 20 tasks in one sitting even with tasks that require fewer decisions because these tasks often require more engagement. 
At some point, participants' interest in accuracy or attention to detail will decline, and the researcher is better off using a larger number of participants completing fewer tasks within the window where participants are attentive and engaged.
As a result, constraints on cognitive load are perhaps the most important factor in determining the sample size necessary for a study.
...
Because there are so many factors which have an effect on the required sample size, explicit power calculations for these experiments can be extremely difficult - experimenters would have to run each experiment once to get the necessary information for a power calculation and then run the **exact same** experiment a second time with the calculated $n$ participants.
Often, after the pilot experiment is completed, more significant changes are necessary that invalidate the estimates which might contribute to the power calculation.
...
When designing graphics experiments, we typically optimize sample size according to constraints identified during the iterative development of the experiment's structure.
First, we optimize the model and data generation method.
Next, we revise the graphics used in the experiment, removing any extraneous information and ensuring that sufficient cues (ticks, grid lines, labels, colors, aesthetics) are provided across experimental conditions to complete the required graphical task.  
Then, we develop the data collection application and determine what types of questions the user will answer during the experiment.
This includes both the primary questions of interest and any secondary measures, such as completion time, participant reasoning, confidence level, and so on. 
As the number of tasks required of the participant increases, the number of trials we can ask participants to complete during a session decreases. 
We pilot test the application, often using a pool of graduate students, colleagues, social media acquaintances, and conscripted family members. 
During the pilot test stage, we pick an optimistic number of trials per participant -- for instance, we might ask pilot participants to evaluate 20 graphical lineups (400 total plots), even though we expect to reduce the number to 10 or 15 during the main experiment. 
After pilot participants complete the study, we ask questions about fatigue to determine what range of trials per participant is reasonable; in order to find this point, pilot testing needs to include more trials than we expect people to complete without fatigue effects. 

At the end of our pilot study, the power calculation becomes an optimization of constraints imposed by the experimental design (e.g. how many blocks in an incomplete block design can we have with the factors under investigation), anticipated analysis method, and budget for the experiment. 
The number of trials a single participant can complete without fatigue affects the block size and strategy by which we allocate trials to each participant.
...
This complex calculus is often not considered in sample size calculations taught in statistics courses, but experimenters do not often have budgets of infinite size to complete experiments, so some financial optimization is necessary in practice. 
This pragmatic sample size calculation may boil down to designing the experiment in a way that allows for collection of data using as many participants as the budget will support. 
...
Online experiments which require more complex engagement with statistical graphics often have between 300 and 600 participants [@robinsonPerceptionCognitiveImplications,], but this varies widely with the experimental design, type of stimuli used, and level of participant engagement required; @loyVariationsQQPlots2016 and @clusters had more than 1300 participants,  while @sineillusion and @vanderplasFramedReproducingRevisiting2019 had fewer than 150 participants. 
In-person experiments [e.g. @vanderplasSpatialReasoningData2016] often use fewer participants than online equivalents, but these experiments may require more tasks of each participant, which reflects the increased logistical costs of scheduling participants, supervising the experiment, and entering the data, if tasks are completed on paper. 
In some cases, in-person experiments may combat cognitive fatigue by scheduling multiple visits per participant; this leaves experiments vulnerable to participant drop-out effects, but can effectively balance cognitive load and participant recruitment costs in some situations.
