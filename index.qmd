...
While it is certainly possible to use equipment such as eye-tracking devices, this paper focuses primarily focus on studies conducted online  without additional equipment.
These experiments have the lowest overhead, offer relatively fast data collection, and provide useful results for well-designed experiments.
...
Visual inference [@bujaStatisticalInferenceExploratory2009;@wickhamGraphicalInferenceInfovis2010] is another useful testing tool for perceptual questions such as "which chart displays this data more clearly" [@power] while simultaneously assessing the significance of the graphical finding in the chart. 
...
While we have not (to date) recorded users talking out loud about what they are seeing, think aloud methods could easily be implemented within a Shiny application, with audio recordings saved to the server for transcription and analysis
[@dunbar1995scientists; @traftonTurningPicturesNumbers2000; @kirschenbaum2003comparative]; it is even possible that these recordings could be automatically transcribed using the speech-to-text functions of large language models.
...
An alternative approach is to analyze the data graphically, as shown in @fig-density-rug, which uses a density plot with rug annotations for point estimates.
When individual response curves are generated, a spaghetti plot with annotations to show anchor points may be useful. 
